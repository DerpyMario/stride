...
// Also covers Dreamcast: this implementation is pure-managed ImageSharp, so it is really the
// portable fallback that Android and iOS replace with their platform decoders, not desktop-only
// code. Kept in this file rather than copied so there is one managed decoder path to maintain.
#if STRIDE_PLATFORM_DESKTOP || STRIDE_PLATFORM_DREAMCAST
